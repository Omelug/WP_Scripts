...
def create_database_if_not_exists():
    try:
        ...
        default_engine = create_engine(conf['DATABASE_URL_ASYNC'].replace("wp_hub_refactored", "postgres").replace("+asyncpg", ""))
        with default_engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
            result = conn.execute(text("SELECT 1 FROM pg_database WHERE datname = 'wp_hub_refactored'"))
            exists = result.fetchone()
            if not exists:
                response = input_cyan("Database 'wp_hub_refactored' does not exist. Do you want to create it? (Y/N): ")
                if response.strip().upper() == 'Y':
                    conn.execute(text("CREATE DATABASE wp_hub_refactored"))
                    print("Database 'wp_hub_refactored' created successfully.")
                else:
